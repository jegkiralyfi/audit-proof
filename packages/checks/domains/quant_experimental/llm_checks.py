...
from packages.checks.domains.quant_experimental.prompts import load_prompt_template
...
def build_instruction(configured_check: PolicyCheck) -> str:
    ...
    if extra:
        return extra
    return f"Assess evidence relevant to quantitative experimental reporting check {configured_check.id}."
...
def runner_context_metadata(configured_check: PolicyCheck) -> dict[str, Any]:
    prompt_name = resolve_prompt_name(configured_check)
    return {
        "prompt_template": f"quant_experimental/{prompt_name}.md" if prompt_name else None,
        "llm_domain": "quant_experimental",
    }
